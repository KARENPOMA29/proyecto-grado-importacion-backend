...
from src.schemas.sucursal import SucursalCreate, SucursalUpdate, SucursalResponse
...
def listar_sucursales(db: Session = Depends(get_db)):
    return sucursal_controller.listar_sucursales(db)
...
def actualizar_sucursal(sucursal_id: int, datos: SucursalUpdate, db: Session = Depends(get_db)):
